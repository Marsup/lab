...
internals.protect = function (item, isTimed, callback) {
    ...
    setImmediate(function () {
        ...
        var context = {
            ...
            plan: function(num) {
                context.plan = null; // Remove myself to avoid multiple calls
                context.expected = num;

                var expect = context.expect;
                context.expect = function() {
                    return expect.apply(null, arguments).with.context(context);
                }

                var assert = context.assert;
                context.assert = function() {
                    context.increment();
                    assert.apply(null, arguments);
                }
                Object.keys(assert).forEach(function(key) {
                    context.assert[key] = function() {
                        context.increment();
        ...
        item.fn.call(context, function (err) {

            if (context.expected) {
                var message = context.counter > context.expected ? 'Too many assertions' : 'Too few assertions';
                Lab.expect(context.counter, message).to.equal(context.expected);
            }
